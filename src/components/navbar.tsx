...
export  function SignInForm({ provider, text, className }: { provider?: string, text?: string, className?: string }) {
    return (
        <form
            action={async () => {
                "use server"
...
export async function SignIn() {
    ...
    if (!session)
        return (<SignInForm className="btn btn-ghost"/>)
    return <div className="btn btn-ghost">{session.user?.name}</div>
}
